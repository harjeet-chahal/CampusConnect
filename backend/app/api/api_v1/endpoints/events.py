    ...
    *,
    ...
    if event.capacity is not None:
        count = db.query(models.EventParticipant).filter(
            models.EventParticipant.event_id == event_id,
            models.EventParticipant.status.in_([schemas.ParticipantStatus.GOING, schemas.ParticipantStatus.INTERESTED])
        ).count()
    ...
    # Send notification
    try:
        await notification_service.send_event_joined(current_user.id, event.title)
    except Exception:
